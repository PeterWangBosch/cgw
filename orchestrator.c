...
static void socket_run_tdr_command(struct mg_connection *nc) {
    FILE *fp;
    char cmd[256] = {0};
    char output[300];
    ...
    strcpy(cmd, "ls -la ");
    if ((fp = popen(cmd, "r")) != NULL) {
        mg_send(nc, "{ \"TDR status\": \"Going to run\"}", 31);
    } else {
        mg_send(nc, "{ \"TDR status\": \"Failed to run\"}", 32);
        return;
    }
...
static void socket_ev_handler(struct mg_connection *nc, int ev, void *p) {
  ...
  switch (ev) {
    ...
    case MG_EV_RECV:
      ...
      } else if (strcmp((char*)io->buf, "/tdr/run") == 0) {
        printf("==> run tdr\n");
        socket_run_tdr_command(nc);
      } else if (strcmp((char*)io->buf, "/tdr/res") == 0) {
